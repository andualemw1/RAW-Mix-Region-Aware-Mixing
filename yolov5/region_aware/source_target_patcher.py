...
class SourceImagePatcher:
    ...
    def __init__(self, src_img, tgt_patch):
        ...
        Args:
            ...
            tgt_patch (torch.Tensor or array-like): Target patch of shape [1, 3, H/2, W/2].
        """
        # Ensure src_img and tgt_patch are torch tensors
        assert isinstance(src_img, torch.Tensor), "src_img must be a torch.Tensor"
        if not isinstance(tgt_patch, torch.Tensor):
            tgt_patch = torch.tensor(tgt_patch, device=src_img.device, dtype=src_img.dtype)
        ...
        assert self.tgt_patch.shape[2] == self.h // 2 and self.tgt_patch.shape[3] == self.w // 2, \
            f"Target patch dimensions mismatch. Expected: [1, 3, {self.h // 2}, {self.w // 2}], Got: {self.tgt_patch.shape}"
